...
plt.figure()
plt.plot(x_euler, U_euler, label=r'$u_{\infty}(x)$', color='tab:green')
plt.plot(x_euler, V_euler, label=r'$v_{\infty}(x)$', color='tab:red')
plt.xlabel(r'$x$')
plt.title(r'FitzHugh-Nagumo Steady-State for $\varepsilon = 0.1$')
plt.legend()
plt.show()
